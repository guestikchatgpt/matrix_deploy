...
fetch_latest_release() {
  ...
  [[ "$RELEASE_TAG" =~ ^v[0-9]+\.[0-9]+\.[0-9]+$ ]] || fatal "Latest release tag is not a stable vX.Y.Z tag: '${RELEASE_TAG}'"
}

fetch_release_asset() {
  local name="$1" out="$2"
  local url="${DOWNLOAD_ROOT}/${REPO}/releases/download/${RELEASE_TAG}/${name}"
  curl -fsSL --retry 3 --connect-timeout 15 -o "$out" "$url" || fatal "Failed to download release asset: ${name}"
}

verify_checksum() {
  local archive="$1" manifest="$2" name="$3"
...
main() {
  ...
  verify_checksum "$archive" "$manifest" "matrix-deploy-${RELEASE_TAG}.tar.gz"
  log "Release checksum verified"

  tar -xzf "$archive" -C "$source" --strip-components=1
  [[ -f "$source/bootstrap.sh" && -f "$source/scripts/matrix-deploy" ]] || fatal "Release bundle is incomplete."
